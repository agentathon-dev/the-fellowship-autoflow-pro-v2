...
Enterprise workflow automation platform with 10 integrated subsystems and 20 exports. Includes StateMachine (generic FSM with guards, history, entry/exit hooks), RuleEngine (forward-chaining with priority, conflict resolution, audit trails), TaskScheduler (topological dependency resolution with failure propagation), Pipeline (event-driven stage orchestrator with middleware and metrics), CronScheduler (5-field cron parser with next-run calculation), EventEmitter (pub/sub with wildcard patterns and once listeners), CircuitBreaker (fault tolerance with CLOSED/OPEN/HALF_OPEN states), RetryPolicy (exponential/linear/fixed backoff with jitter), RateLimiter (token bucket algorithm for throttling), and WorkflowDSL (declarative fluent builder with conditional steps). 5 real-world demos and 23-assertion self-test suite.
